...
from chart_pretrain_data import get_loader
...
# def loadT5_weights(model, T5model):
#     #load the Encoder with the shared name
...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        from chart_pretrain_model import VLT5Pretraining, VLBartPretraining
        ...
        #     self.model.load_state_dict(T5model.state_dict())
    ...
    def predict(self, loader):
        self.model.eval()
        #assert 'table_lm' in self.args.losses
        #run the prediction
        with torch.no_grad():
            uid2tab = {}
            if self.verbose:
                pbar = tqdm(total=len(loader), ncols=250, desc="Prediction")
            for i,batch in enumerate(loader):
                ...
                if self.args.distributed:
                    ...
                    results = self.model.valid_step(batch)
                #print(results.keys())
                if 'table_lm' in self.args.losses or 'tsp' in self.args.losses or 'tdp' in self.args.losses:
                    table_pred = results['table_pred']
                    for uid, source_text, target_text, pred_tab in zip(batch['uid'],batch['source_text'], batch['target_text'],table_pred): 
                        uid2tab[uid] = (source_text, target_text, pred_tab)
        ...
        uid2tab_list = dist_utils.all_gather(uid2tab)
        uid2tab_result = {}
        if self.verbose:
            for uid2tab in uid2tab_list:
                for k, v in uid2tab.items():
                    uid2tab_result[k] = {"source_text": v[0], "target_text": v[1], "pred_tab": v[2]}
        return uid2tab_result
...
def main_worker(gpu, args):
    ...
    #trainer.train()
    #Predict the result
    #trainer.evaluate_epoch(test_loader)
    uid2tab_result = trainer.predict(test_loader)

    uid2tab_new = {}
    for key, val in uid2tab_result.items():
        uid2tab_new[str(key)] = str(val)
    with open(args.output + 'predictions.json', 'w') as f:
        json.dump(uid2tab_new, f)
