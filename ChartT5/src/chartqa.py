...
from chartqa_data import get_loader
...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        from chartqa_model import VLT5ChartQA, VLBartChartQA

        model_kwargs = {}
        if 't5' in args.backbone:
            model_class = VLT5ChartQA
        elif 'bart' in args.backbone:
            model_class = VLBartChartQA
    ...
    def train(self):
        if self.verbose:
            ...
            numerical_loss_meter = LossMeter()
            best_valid = 0.
            best_epoch = 0
        ...
        for epoch in range(self.args.epochs):
            ...
            for step_i, batch in enumerate(self.train_loader):
                ...
                loss = results['loss']
                if 'numerical_loss' in results:
                    numerical_loss = results['numerical_loss']
                    numerical_loss.detach()

                #print(results['numerical_loss'])
                if self.args.fp16 and _use_native_amp:
                ...
                if self.verbose:
                    ...
                    if 'numerical_loss' in results:
                        numerical_loss_meter.update(numerical_loss.item())
                        desc_str += f' | Numerical Loss {numerical_loss_meter.val:4f}'
            ...
            if self.verbose:
                ...
                if 'numerical_loss' in results:
                    wandb_log_dict['Train/Numerical_Loss'] = epoch_results['numerical_loss'] / len(self.train_loader)
                wandb_log_dict['Valid/score'] = valid_score_raw
        ...
        # with open('/'.join([args.output, 'best_predictions.json']), 'w') as f:
