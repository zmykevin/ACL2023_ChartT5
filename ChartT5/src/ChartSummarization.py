...
from ChartSumm_data import get_loader
...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        self.model.decoder.load_state_dict(T5model.decoder.state_dict())
    ...
    def train(self):
        ...
        #Run evaluation here:
        valid_results = self.evaluate(self.val_loader, epoch=-1, split="val")
        for epoch in range(epochs):
            ...
            # Validation
            self.save(self.args.dataset_name+"_ep_"+str(epoch)+"_2ndrun")
            if epoch>13:
                valid_results = self.evaluate(self.val_loader, epoch=epoch, split='val')
                ...
                if valid_score > best_valid or epoch == 0:
                    ...
                    best_epoch = epoch
                    self.save("BEST_"+self.args.dataset_name)
        ...
        for ep_result in self.log_msg:
            print(ep_result)
        
        # Test Set
        best_path = os.path.join(self.args.output, "BEST_"+self.args.dataset_name)
        self.load(best_path, loc='cuda:0')

        if self.verbose:
            wandb.save(best_path, base_path=self.args.output)
            print(f'\nUploaded checkpoint {best_epoch}', best_path)

        test_results = self.evaluate(self.test_loader, epoch="test", split='test')
    ...
    def predict(self, loader, dump_path=None, split="val"):
        """
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(tqdm(loader, ncols=120, desc="Prediction", disable=not self.verbose)):
                ...
                if self.args.ocr_copy:
                    for img_id,pred in zip(batch['img_id'], results['pred']):
                        with open(loader.dataset.chartsum_dir.joinpath(f"{split}/ocr_results/"+img_id+".json"), "r") as f:
                            obj_ocrs = json.load(f)
                        gt_ocrs =  [ocr[0] for bbox, ocr in obj_ocrs]
                        if re.search(u"ocr_\d+>", pred):
                            match = re.findall(u"ocr_\d+>", pred)
                            for m in match:
                                matched_index = int(re.findall(u'\d+',m)[0])
                                try:
                                    matched_ocr = gt_ocrs[matched_index]
                                    pred = pred.replace(m, matched_ocr)
                                    predictions.append(pred)
                                    fnames.append(img_id)
                                except:
    ...
    def evaluate(self, loader, dump_path=None, epoch=10, split="vals"):
        results = self.predict(loader, dump_path, split=split)
        if self.verbose:
            predictions, targets = results['predictions'], results['targets']
            predictions, targets = [x[0] for x in predictions], [x[0] for x in targets]
            #print('# predictions:', len(predictions))
            if not os.path.exists("yi_results"):
                os.mkdir("yi_results")
            with open("yi_results/"+self.args.dataset_name+"_ep_"+str(epoch)+"_.json", "w") as f:
                json.dump(results, f)
            p = subprocess.Popen(['conda','run','-n', 'vlt5_eval',\
                    'python','VL-T5/src/ChartSumm_eval.py',self.args.dataset_name,str(epoch),\
                    "test" if str(epoch)=="test" else "val"],\
                    stdin=subprocess.PIPE, stdout=subprocess.PIPE)
            output, _ = p.communicate()
            bleu, cs, bleurt, cider, ppl = output.decode('utf-8').split("BLeU,CS,BLEURT,CIDER,PPL")[-1].split()
            print(epoch, "BLeU,CS,BLEURT,CIDER,PPL",bleu, cs, bleurt, cider, ppl)
            self.log_msg.append((epoch, "BLeU,CS,BLEURT,CIDER,PPL",bleu, cs, bleurt, cider, ppl))
            valid_result = {"Bleu":bleu, "CS":cs, "Bleurt":bleurt, "PPL":ppl}
            valid_result["CIDEr"] = float(cider)
            return valid_result
            if False: #dump_path is None:
...
if __name__ == "__main__":
    ...
    # print("args load should be: {}".format(args.load))
    ...
    # args.gpu = "cuda:3"
    # args.batch_size = 32
    args.max_text_length = int(5*args.max_text_length)
    args.gen_max_length = int(5*args.gen_max_length)
    #args.oscar_tags = True
    # args.num_beams = 4
    if args.distributed:
