...
class VisualOCREmbedding(nn.Module):
    def __init__(self, config, word_embedding):
        ...
        if self.config.individual_vis_layer_norm:
            ...
            if self.config.use_vis_order_embedding:
                ...
                self.img_order_embedding = nn.Embedding(n_images, config.d_model)
            self.word_embedding = word_embedding
            #get the ocr_embedding
            self.ocr_embedding = nn.Sequential(*[self.word_embedding,T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)])
    ...
    def forward(self, feats, pos, visocrs=None, visocrs_pos=None, img_order_ids=None, obj_order_ids=None):
        """
        Args
            feats: [B, N, feat_dim]
            pos: [B, N, 4]
        ...
        if self.config.use_vis_order_embedding:
            ...
            if obj_order_ids is None:
                ...
                obj_order_ids = obj_order_ids.unsqueeze(0) #.expand(B,-1)
            # assert obj_order_ids.max().item() < 32200, obj_order_ids
            obj_order_ids = self.word_embedding.num_embeddings - obj_order_ids - 1
            obj_order_embedding = self.word_embedding(obj_order_ids)
        ...
        if visocrs is not None:
            _,N_OCR = visocrs.size()
            assert visocrs_pos.size() == (B, N_OCR, 4)
            visocrs_area = self.get_area(visocrs_pos).unsqueeze(2) # [B,N_OCR, 1]
            visocrs_pos = torch.cat([visocrs_pos, visocrs_area],dim=2) #[B, N_OCR, 5]
            
            visocr_word_embedding = self.ocr_embedding(visocrs)
            absolute_visocrs_pos_embedding = self.absolute_vis_pos_embedding(visocrs_pos)

            visocr_embedding =visocr_word_embedding + absolute_visocrs_pos_embedding
        else:
            visocr_embedding = None

        # print(visocr_embedding.size())
        # print(vis_embedding.size())
        joint_embedding = torch.cat([x for x in [vis_embedding, visocr_embedding] if x is not None], dim=1)
        # print(joint_embedding.size())
        return joint_embedding


#Implement the ChartT5_JointEncoder
class ChartT5_JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None):
        super(T5Stack, self).__init__(config)
        self.config = config
        ...
        #self.visual_embedding = VisualOCREmbedding(self.config, embed_tokens)
        self.visual_embedding = VisualEmbedding(self.config, embed_tokens)
        self.block = nn.ModuleList(
            [T5Block(config, has_relative_attention_bias=(i == 0))
                for i in range(config.num_layers)]
        ...
        #Get an Numerical Embedding
        self.mantissa_embedding = nn.Sequential(*[nn.Linear(1, config.d_model),T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)])
        
        self.exponent_embedding = nn.Sequential(*[nn.Linear(1, config.d_model),T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)])

        #self.ocr_embedding = nn.Sequential(*[self.embed_tokens,T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)])
    def set_input_embeddings(self, new_embeddings):
        self.embed_tokens = new_embeddings
        self.visual_embedding.obj_order_embedding = new_embeddings
    ...
    def forward(
        ...
        #include visocrs
        if visocrs_inputs is not None:
            # print("separate ocr embedding is activated")
            visocrs = visocrs_inputs[0]
            visocrs_bboxes = visocrs_inputs[1]

            visocrs_area = self.visual_embedding.get_area(visocrs_bboxes).unsqueeze(2) # [B, N, 1]
            visocrs_bboxes = torch.cat([visocrs_bboxes, visocrs_area], dim=2) # [B, N, 5]
        else:
            visocrs = None
            visocrs_bboxes = None
        
        #Get the input numericals
        if input_numericals is not None:
            if input_numericals[0] is not None:
                input_mantissa = input_numericals[0]
                input_exponent = input_numericals[1]
                #print(input_mantissa.size())
        else:
            input_numericals = (None, None)
        
        if visocrs_numericals is not None:
            if visocrs_numericals[0] is not None:
                visocrs_mantissa = visocrs_numericals[0]
                visocrs_exponent = visocrs_numericals[1]
        else:
            visocrs_numericals = (None, None)
        ...
        # visocr_embeds = None
        # V_L_OCR = 0
        #print(input_numericals)
        # input_ids = torch.cat([visocrs, input_ids], dim=1)
        if inputs_embeds is None:
            assert self.embed_tokens is not None, "You have to initialize the model with valid token embeddings"
            inputs_embeds = self.embed_tokens(input_ids)
            if input_numericals[0] is not None:
               inputs_mantissa_embedding = self.mantissa_embedding(input_numericals[0])
               inputs_exponent_embedding = self.exponent_embedding(input_numericals[1])
               inputs_embeds = inputs_embeds + inputs_mantissa_embedding + inputs_exponent_embedding
        ...
        #update vis_embeds
        # vis_embeds = self.visual_embedding(
        #     vis_feats, boxes, visocrs, visocrs_bboxes, img_order_ids, obj_order_ids)
        ...
        if visocrs is not None:
            # _,N_OCR = visocrs.size()
            # assert visocrs_pos.size() == (B, N_OCR, 4)
            #visocrs_area = self.visual_embedding.get_area(visocrs_bboxes).unsqueeze(2) # [B,N_OCR, 1]
            #visocrs_bboxes = torch.cat([visocrs_bboxes, visocrs_area],dim=2) #[B, N_OCR, 5]
            
            visocr_word_embedding = self.embed_tokens(visocrs)
            absolute_visocrs_pos_embedding = self.visual_embedding.absolute_vis_pos_embedding(visocrs_bboxes)

            visocr_embeds =visocr_word_embedding + absolute_visocrs_pos_embedding
            #visocr_embeds =visocr_word_embedding
            if visocrs_numericals[0] is not None:
               visocr_mantissa_embedding = self.mantissa_embedding(visocrs_numericals[0])
               visocr_exponent_embedding = self.exponent_embedding(visocrs_numericals[1])
               #print(visocr_mantissa_embedding)
               visocr_embeds = visocr_embeds + visocr_mantissa_embedding + visocr_exponent_embedding
        else:
            visocr_embeds = None
        
        #print(visocr_embeds)
        inputs_embeds = torch.cat([x for x in [inputs_embeds, visocr_embeds, vis_embeds] if x is not None], dim=1)
        #inputs_embeds = torch.cat([x for x in [inputs_embeds, vis_embeds] if x is not None], dim=1)
        ...
        if vis_attention_mask is None:
            vis_attention_mask = attention_mask.new_ones(B, V_L)
        #add the visocrs_attention_mask
        if visocrs_attention_mask is None and visocr_embeds is not None:
            V_L_OCR = visocr_embeds.size(1)
            #visocrs_attention_mask = attention_mask.new_ones(B, V_L_OCR)
            visocrs_attention_mask = visocrs.ne(self.config.pad_token_id).to(dtype=inputs_embeds.dtype, device=inputs_embeds.device)
        else:
            V_L_OCR = 0

        attention_mask = torch.cat([x for x in [attention_mask, visocrs_attention_mask, vis_attention_mask] if x is not None], dim=1)
        #attention_mask = torch.cat([x for x in [attention_mask, vis_attention_mask] if x is not None], dim=1)
        # ourselves in which case we just need to make it broadcastable to all heads.
        extended_attention_mask = self.get_extended_attention_mask(
            attention_mask,
            (B, L+V_L_OCR+V_L),
        ...
        if self.config.num_layers > 0:
            ...
            # print(text_position_bias.size())
            position_bias[:, :, :L+V_L_OCR, :L+V_L_OCR] = text_position_bias
...
#Implement the Chart T5
class ChartT5(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        self.encoder = ChartT5_JointEncoder(encoder_config, self.shared)
        #------------------#
        ...
        #Add the Output Head, Mingyang Zhou
        self.num_mantissa_head = nn.Linear(config.d_model, 1, bias=False)
        self.num_exponent_head = nn.Linear(config.d_model, 1, bias=False)
    ...
    def forward(
        ...
        use_cache=None,
        labels=None,
        labels_numericals=None,
        labels_numericals_mask = None,
        ...
        **kwargs,
        ...
        #print(visocrs_inputs)
        if encoder_outputs is None:

            encoder_outputs = self.encoder(
                input_ids=input_ids,
                input_numericals=input_numericals,
                attention_mask=attention_mask,
                inputs_embeds=inputs_embeds,

                vis_inputs=vis_inputs,
                vis_attention_mask=vis_attention_mask,
                visocrs_inputs = visocrs_inputs,
                visocrs_numericals = visocrs_numericals,
                visocrs_attention_mask = visocrs_attention_mask,
        ...
        if labels is not None:
            ...
            if reduce_loss:
                if lm_label_weights is not None:
                    loss_fct = CrossEntropyLoss(weight=lm_label_weights, ignore_index=-100)
                else:
                    loss_fct = CrossEntropyLoss(ignore_index=-100)
            else:
                if lm_label_weights is not None:
                    loss_fct = CrossEntropyLoss(weight=lm_label_weights, ignore_index=-100, reduction='none')
                else:
        ...
        #print(loss.size())
        if labels_numericals is not None:
            if labels_numericals[0] is not None:
                #Compute the Output
                mantissa_logits = self.sigmoid_f(self.num_mantissa_head(sequence_output))
                exponent_logits = self.sigmoid_f(self.num_exponent_head(sequence_output))
                
                mantissa_target = labels_numericals[0]
                exponent_target = labels_numericals[1]
                
                # print(mantissa_target.size())
                assert labels_numericals_mask is not None

                num_loss_fct = MSELoss(reduction='none')

                mantissa_loss = num_loss_fct(torch.flatten(labels_numericals_mask*mantissa_logits), torch.flatten(mantissa_target))
                exponent_loss = num_loss_fct(torch.flatten(labels_numericals_mask*exponent_logits), torch.flatten(exponent_target))
                
                
                #print(mantissa_loss.size())
                #loss = loss + mantissa_loss + exponent_loss 

                numerical_loss = mantissa_loss + exponent_loss
        ...
        if return_numerical_value:
            mantissa_logits = self.sigmoid_f(self.num_mantissa_head(sequence_output))
            exponent_logits = self.sigmoid_f(self.num_exponent_head(sequence_output))
            # print(sequence_output.size())
            return VLSeq2SeqLMOutput(
            numerical_mantissa_prediction = mantissa_logits,
            numerical_exponent_prediction = exponent_logits,
            )
        # if not return_dict:
...
class VLSeq2SeqLMOutput(ModelOutput):
    ...
    numerical_loss: Optional[torch.FloatTensor] = None #Added by Mingyang
    ...
    numerical_mantissa_prediction: Optional[torch.FloatTensor] = None
    numerical_exponent_prediction: Optional[torch.FloatTensor] = None
