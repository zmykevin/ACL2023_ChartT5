...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataroot', type=str,
                        default='/dvmm-filer2/projects/mingyang/semafor/chart_pretrain')
    ...
    #chartsum_dir = Path(args.outroot).resolve()
    chartsum_dir = Path(args.dataroot).resolve()
    chartsum_img_dir = chartsum_dir.joinpath(SPLIT2DIR[args.split]).joinpath("images")
    #chartsum_img_dir = chartsum_dir.joinpath(SPLIT2DIR[args.split]).joinpath("png")

    dataset_name = 'ChartPretrain'

    out_dir = chartsum_dir.joinpath('features')
    #out_dir = Path('/dvmm-filer2/projects/mingyang/semafor/chart_pretrain/plotqa/features').resolve()
    ...
    dataset = ChartSumDataset(chartsum_img_dir)
